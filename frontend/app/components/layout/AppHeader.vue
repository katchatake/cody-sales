    ...
    <v-menu>
      <template v-slot:activator="{ props }">
        <ClientOnly>
          ...
          <template #fallback>
            <v-avatar color="brand-primary" class="cursor-pointer shadow-base border" size="44" v-bind="props">
              <span class="font-weight-bold text-surface-base">U</span>
            </v-avatar>
          </template>
        </ClientOnly>
      </template>
      <v-list class="mt-2 rounded-xl shadow-lg border-0 pa-2" min-width="220" elevation="4">
        <ClientOnly>
          ...
          <template #fallback>
            <div class="px-4 py-2">
              <div class="text-body-md font-weight-bold">Usuario</div>
              <div class="text-caption text-muted">user@domain.com</div>
            </div>
          </template>
        </ClientOnly>
